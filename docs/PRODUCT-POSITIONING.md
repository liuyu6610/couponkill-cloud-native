...
| API | `GET /api/v1/connector/price-compare?couponId=`（已落地：主绑定 probe + `coupon_price_map` 手工映射；admin：`POST/GET/DELETE /price-maps`） |
| 不做 | 保证最低价下单；历史价曲线（P2） |
| 模块 | `connector-service` + `frontend`（券详情「同品参考价」、秒杀成功 CTA、订单详情入口、ConnectorAdmin 映射） |
