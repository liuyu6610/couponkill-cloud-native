...
/**
 * 内网 Token 门禁：prod/strict 禁止默认弱口令；其它环境强告警。
 */
@Slf4j
@Component
public class InternalTokenGuard implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (isDefault) {
            log.warn("【安全】connector.internal-token 仍为默认值 couponkill-internal，仅限本地；生产请设置 CONNECTOR_INTERNAL_TOKEN 并开启 connector.internal-token-strict=true");
        }
    }
}
