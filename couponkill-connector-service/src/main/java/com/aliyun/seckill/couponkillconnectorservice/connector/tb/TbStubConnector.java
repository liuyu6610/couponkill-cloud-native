package com.aliyun.seckill.couponkillconnectorservice.connector.tb;
...
/**
 * 淘宝开放平台 Connector 预留 stub（二期实现）。
 * 不进秒杀热路径；health 恒为 DISABLED，避免误绑后假装成功。
 */
@Component
public class TbStubConnector implements EcommerceConnector {
    ...
    public PlatformProductSnapshot getProduct(String externalSkuId) {
        throw new UnsupportedOperationException("淘宝 Connector 二期未实现，请使用 MOCK/JD");
    }
    ...
    public ConnectorHealth health() {
        return ConnectorHealth.builder()
                ...
                .status("DISABLED")
                .message("淘宝 Connector 预留（二期），SPI 已挂载")
                .build();
    }
}
