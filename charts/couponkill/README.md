...
- Connector 服务 (couponkill-connector-service，旁路库存同步，不进热路径)
- Go 秒杀服务 (seckill-go-service，默认关闭)
...
### 依赖服务管理
支持 PostgreSQL、Redis、Kafka（KRaft）、Nacos 和 Sentinel 等依赖服务的部署和初始化。
历史 MySQL / RocketMQ 已迁移，见仓库根目录 `docs/MIGRATION-PostgreSQL-Kafka.md`。

### 动态配置和集群支持
支持中间件的动态扩展和集群模式，包括：
- PostgreSQL 单节点 / 主从
- Redis 集群 / 哨兵
- Kafka 集群（分区真源：`values.yaml` → `kafka.partitions`）
- Nacos 集群模式
...
│  依赖服务模块 (可选)                                                        │
│  ├── Nacos (服务注册与配置中心)                                             │
│  ├── Kafka KRaft (消息队列，替代 RocketMQ)                                  │
│  ├── PostgreSQL (关系型数据库，替代 MySQL)                                  │
│  ├── Redis (缓存)                                                           │
│  └── Sentinel (流量控制)                                                    │
├─────────────────────────────────────────────────────────────────────────────┤
│  扩展功能模块                                                               │
...
> **生产务必先执行上面的 `kubectl create secret`，再 helm。** `values-prod.yaml` 已 `secrets.create=false`，Chart 不会代建 Secret。  
> P0 轮换清单（Apifox / ACR / RDS·Redis 白名单）、key 与环境变量对照、滚动更新、以及残留风险（Java root、Istio `tls:false`、`latest`）见 [`docs/SECURITY-ROTATION.md`](../../docs/SECURITY-ROTATION.md)；部署路径真源见 [`docs/DEPLOYMENT-SOURCE-OF-TRUTH.md`](../../docs/DEPLOYMENT-SOURCE-OF-TRUTH.md)。
...
middleware:
  postgres:
    # 多节点列表（外部托管 PG / 自建 StatefulSet 时填写）
    cluster:
      enabled: false
      # 节点列表格式: host:port
      # 示例:
      # nodes:
      #   - "postgres-0.postgres-headless:5432"
      #   - "postgres-1.postgres-headless:5432"
      nodes: []

    # 流复制主从（只读流量可指从库；写仍走 master）
    replication:
      enabled: false
      master:
        host: ""
...
#### Kafka 配置模板（KRaft，无需 ZooKeeper / RocketMQ）
...
db:
  ...
  cluster:
    ...
    nodes:
      - "postgres-0.postgres-headless:5432"
      - "postgres-1.postgres-headless:5432"
      - "postgres-2.postgres-headless:5432"
```
